            ...
            for i in range(self.cycle_stop):
                ...
                if step % 10 == 0:
                    ...
                    if acc_image > self.acc_stop:
                        saver.save(sess, self.model_save_dir + self.model_save_name)
...
def main():
    train_image_dir = CAPTCHA_CONFIG["train_image_dir"]
    verify_image_dir = CAPTCHA_CONFIG["test_image_dir"]
    model_save_dir = CAPTCHA_CONFIG["model_save_dir"]
    model_save_name = CAPTCHA_CONFIG["model_save_name"]
    cycle_stop = CAPTCHA_CONFIG["cycle_stop"]
    acc_stop = CAPTCHA_CONFIG["acc_stop"]
    cycle_save = CAPTCHA_CONFIG["cycle_save"]
    enable_gpu = CAPTCHA_CONFIG["enable_gpu"]
    image_suffix = CAPTCHA_CONFIG['image_suffix']
    use_labels_json_file = CAPTCHA_CONFIG['use_labels_json_file']
    train_batch_size = CAPTCHA_CONFIG['train_batch_size']
    test_batch_size = CAPTCHA_CONFIG['test_batch_size']
    ...
    tm = TrainModel(train_image_dir, verify_image_dir, char_set, model_save_dir, model_save_name, cycle_stop, acc_stop,
                    cycle_save, image_suffix, train_batch_size, test_batch_size, verify=False)
    tm.train_cnn()  # 开始训练模型
