...
def verify(origin_dir, real_width, real_height, image_suffix):
    ...
    for index, img_name in enumerate(img_list):
        ...
        if real_size == img.size:
            ...
            img.resize(real_size).save(file_path)
            print(f"图片 {img_name} 尺寸异常：{img.size}，已修改为尺寸：{real_size}")
...
def main():
    # 图片路径
    origin_dir = CAPTCHA_CONFIG["origin_image_dir"]
    new_dir = CAPTCHA_CONFIG["new_image_dir"]
    train_dir = CAPTCHA_CONFIG["train_image_dir"]
    test_dir = CAPTCHA_CONFIG["test_image_dir"]
    # 图片尺寸
    real_width = CAPTCHA_CONFIG["image_width"]
    real_height = CAPTCHA_CONFIG["image_height"]
    # 图片后缀
